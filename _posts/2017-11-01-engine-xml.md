...
# Approach
We observed that many of our features required changes on the client side, either in the UI or REST-API, as well as on the back-end side but only minimal changes were needed on the agent side. The latter part mostly included conversions of data into Libvirt's XMLs.

Thus, by letting ovirt-engine speak the language of Libvirt rather than that of VDSM, i.e., converting its business entities directly to Libvirt's XMLs, not only that we use a more standard API in ovirt-engine but we avoid an often redundant conversion of data on the host. Furthermore, this reduces the chances for people making hacks on the host side and increases the chance that the representation of the data in ovirt-engine will be aligned with the one in Libvirt.
